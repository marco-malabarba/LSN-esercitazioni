...
stamparisultati((string) "ground_state_2.dat",media_cumulativa,errore,N);
...
double psi (double x, double mu, double sigma) {
	return exp(-sigma*abs(pow(x-mu,3.)))+exp(-sigma*abs(pow(x+mu,3.)));
}
...
double diff_2_psi (double x, double mu, double sigma) {
	double primo_fattore=-6*abs(x+mu)*sigma*exp(-sigma*abs(pow(x+mu,3.)))-6*abs(x-mu)*sigma*exp(-sigma*abs(pow(x-mu,3.)));
	double secondo_fattore=9*sigma*sigma*pow(x+mu,4)*exp(-sigma*abs(pow(x+mu,3.)))+9*sigma*sigma*pow(x-mu,4)*exp(-sigma*abs(pow(x-mu,3.)));
...
void cercaparametri (double &mu,double &sigma,double &E_min,double &x,Random &rnd, int M) {
	for (double mu_new=0.1; mu_new<=1.; mu_new+=0.1) {
		for (double sigma_new=0.1; sigma_new<=3.; sigma_new+=0.1) {
