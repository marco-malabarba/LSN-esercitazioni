...
double external_potential(double val)
{
	...
	return pow(val,4.)-5.*val*val/2.0;
}
...
double variationalWaveFunction_second(double v)
{
	...
	double mu=0.6184;
	//return 0;
	return ((pow(v+mu,2)*exp(-(v+mu)*(v+mu)/(2*sigma*sigma))+pow(v-mu,2)*exp(-(v-mu)*(v-mu)/(2*sigma*sigma)))/(sigma*sigma)-variationalWaveFunction(v))/(sigma*sigma);
}
