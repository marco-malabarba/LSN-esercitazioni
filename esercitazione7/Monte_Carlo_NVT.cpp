...
//Read initial configuration
  ...
  if (restart==0) ReadConf.open("config.0");
  else ReadConf.open("config.final");
...
// contribution to energy and virial
  ...
  if (istantanei==1) {
    ofstream out("istantanei_energia.dat",ios::app);
    out<<walker[iv]/(double)npart + vtail<<endl;
    out.close();
    out.open("istantanei_pressione.dat", ios::app);
    out<<rho * temp + (walker[iw] + (double)npart * ptail) / vol<<endl;
  }
}
...
//g(r)
    for (int k=igofr; k<igofr+nbins; k++) {
      double valore=blk_av[k]/(blk_norm*rho*npart*4./3.*M_PI*(pow((bin_size)*(k-2.+1.),3.)-pow(bin_size*(k-2.),3.)));
      Gofr<<valore<<" ";
      glob_av[k]+=valore;
      glob_av2[k]+=valore*valore;
    }
...
void finalgr(void) {
  ...
  for (int k=igofr; k<igofr+nbins; k++) {
    ...
    Gave<<(k-2)*bin_size<<" "<<glob_av[k]/(double)nblk<<" "<<err_gdir<<endl;
  }
  Gave.close();
  return;
}
