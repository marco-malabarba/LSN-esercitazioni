...
void popolazione::sim_ann(double temp) {
	...
	if (rnd.Rannyu()<alfa) { //non faccio niente, accetto la mossa proposta, ma tecnicamente ho già modificato l'individuo
	}
	else {
		for (int i=0; i<m_n_cities; i++) m_individui[0][i]=perc_old[i]; //rifiuto, torno alla configurazione precedente 
		calcolalunghezze();
